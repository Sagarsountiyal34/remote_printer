...
class UploadDocument
  ...
  def get_cloned_file_absolute_path
    file_src = self.get_absolute_path
    ext = File.extname(file_src)
    file_src.sub(ext, '_group_otp' + ext)
  end

  def get_cloned_pdf_file_absolute_path
    file_src = self.get_absolute_path
    ext = File.extname(file_src)
    file_src.sub(ext, '_group_otp' + '.pdf')
  end
  ...
  def add_documents(group)
      ...
      if FileInfo.is_image?(self.document_url)
          ...
          cloned_document_data['metadata']['size'] = File.size(self.get_cloned_file_absolute_path)
          cloned_document_data['id'] = File.basename(self.get_cloned_file_absolute_path)
      else
          cloned_document_data['metadata']['mime_type'] = "application/pdf"
          cloned_document_data['metadata']['filename'] = file_name.sub(ext, '.pdf')
          cloned_document_data['metadata']['size'] = File.size(self.get_cloned_pdf_file_absolute_path)
          cloned_document_data['id'] = File.basename(self.get_cloned_pdf_file_absolute_path)
      end
  ...
  def add_otp_for_pdf(otp)
    pdf_src = self.get_cloned_pdf_file_absolute_path
  ...
  def add_otp_for_image(otp)
      img_src = self.get_cloned_file_absolute_path
  ...
  def create_pdf_from_file
    docx_src = self.get_cloned_file_absolute_path
  ...
  def generate_deep_copy_in_directory
      src_path = self.get_absolute_path
      ext = File.extname(src_path)
      dest_src = src_path.sub(ext, '_group_otp' + ext)
