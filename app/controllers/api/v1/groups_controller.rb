class Api::V1::GroupsController < ApplicationApiController
    ...
    def get_groups_wd_status
	    begin
	      ...
	      if groups_status=="all"
	        groups = @current_company.groups.where(status: "ready_for_print").map{|g| g.attributes.merge(user_email: g.user.email,total_cost: group_total(g),note_text_present: g.user.note.try(:note_text).present?, net_cost: get_net_cost_of_groups(g))}
	      else
    ...
    def get_groups(groups_status)
        return  @current_company.groups.where(status: "ready_for_print").all_of({:'documents.status' => groups_status }).map{|grp| grp.attributes.merge(documents: grp.documents.where(status: groups_status),user_email: grp.user.email,total_cost: group_total(grp),note_text_present: grp.user.note.try(:note_text).present?)}
    end
