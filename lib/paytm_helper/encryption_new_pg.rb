...
module PaytmHelper::EncryptionNewPG
  ...
  PaymentData = YAML.load_file(File.join(Rails.root, 'config', 'payment_secret.yml'))
  ...
  def prepare_payment_params(params)
      payment_environment = 'staging'
      @param_list = Hash.new  #1
      order_id = params['payment_detail_id']
      cust_id = current_user.id
      txn_amount = params['amount']
      email = current_user.email.present? ? current_user.email : ''
      mobile_no = current_user.phone_number.present? ? current_user.phone_number : ''

      @param_list["MID"] = PaymentData['paytm_credentials'][payment_environment]['mid']
      # @param_list["PAYMENT_MODE_ONLY"] = "YES"
      # @param_list["PAYMENT_TYPE_ID"] = "UPI"
      @param_list["ORDER_ID"] = order_id
      @param_list["CUST_ID"] = cust_id
      @param_list["INDUSTRY_TYPE_ID"] = PaymentData['paytm_credentials'][payment_environment]['industry_type_id']
      @param_list["CHANNEL_ID"] = PaymentData['paytm_credentials'][payment_environment]['web']['channel_id']
      @param_list["TXN_AMOUNT"] = txn_amount
      @param_list["MOBILE_NO"] = mobile_no
      @param_list["EMAIL"] = email
      @param_list["WEBSITE"] = PaymentData['paytm_credentials'][payment_environment]['web']['website']
      @param_list["CALLBACK_URL"] = PaymentData['paytm_credentials'][payment_environment]['web']['callback_url']

      puts "---#{@param_list}---"
      puts "---#{PaymentData['paytm_credentials'][payment_environment][:merchant_key]}---"
      @checksum_hash = new_pg_checksum(@param_list, PaymentData['paytm_credentials'][payment_environment]['merchant_key']).gsub("\n",'') #2

      puts "param_list: #{@param_list}"
      puts "CHECKSUMHASH: #{@checksum_hash}"

      @payment_url = PaymentData['paytm_credentials'][payment_environment]['web']['payment_url'] #3
      return [@param_list, @checksum_hash, @payment_url]
  end
end